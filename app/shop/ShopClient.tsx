...
function ActiveChips({ filters, search, onChange, onClearSearch, onReset }: {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-5">
      ...
      {chips.length > 1 && (
        <button onClick={onReset} suppressHydrationWarning className="text-[11px] text-gray-400 hover:text-red-500 underline underline-offset-2 transition-colors">
          Tout effacer
        </button>
      )}
    </div>
...
export default function ShopClient({ initialProducts }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return r;
  }, [initialProducts, search, filters, sort]);
      ...
      <div className="border-b border-gray-100 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-3">
          ...
          <div className="flex-shrink-0">
            <h1 className="font-display text-xl sm:text-2xl tracking-tight leading-none">
              {filters.department.length === 1
                ? DEPARTMENT_LABELS[filters.department[0]]
                : 'Boutique'}
            </h1>
            <p className="text-xs text-gray-400 mt-0.5">
              {`${filtered.length} article${filtered.length !== 1 ? 's' : ''} — Page ${page} / ${totalPages || 1}`}
            </p>
          </div>
